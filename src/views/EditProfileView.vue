...
export default {
  ...
  methods: {
    editProfile() {
      ...
      if (
        !document.getElementById("name-input").value.match(/^[a-zA-Z0-9]+$/)
      ) {
      ...
      this.$router.push(
        "/profile/" + JSON.parse(localStorage.getItem("userInfo"))[0].id
      );
    },
    deleteProfile() {
      //Getting the user's token
    ...
    <section id="forms-container">
      ...
      <div class="single-form">
        ...
        <input
          ...
          class="field"
          type="text"
          placeholder="www.salleurl.edu/salle_en.png"
        />
      </div>

      <nav id="nav-delete-account" v-on:click="deleteProfile()">
        Delete account
      </nav>

      <div class="button-container">
        <button v-on:click="editProfile()" class="save-button primary-button">
          SAVE CHANGES
        </button>
        <button
