    ...
    fetch(
      ...
      .then((res) => {
        if (res) {
          ...
          res[0].avg_score == null
            ? (res[0].avg_score = 0)
            : (this.rating = res[0].avg_score);
          res[0].num_comments == null
            ? (res[0].num_comments = 0)
            : (this.comments = res[0].num_comments);
          res[0].percentage_commenters_below == null
            ? (res[0].percentage_commenters_below = 0)
...
<template>
  <div id="profile-main">
    ...
    <section id="container-full-profile">
      <nav>
        <RouterLink id="nav-timeline" to="/timeline" v-if="show"
          >View timeline</RouterLink
        >
      </nav>
    </section>
    ...
    <section id="container-full-profile">
      <div class="button-container">
        <a href="/login" v-if="show">
          <button v-on:click="logout()" class="logout-button primary-button">
            LOGOUT
          </button>
        </a>
        <button
          v-on:click="followUser()"
          class="follow-button primary-button"
          v-if="!show && !showFollow"
        >
          FOLLOW
        </button>
        <a href="/editProfile">
          <button a class="edit-button secondary-button" v-if="show">
            EDIT
          </button>
        </a>
      </div>
